...
# .env 또는 settings.py에 FINLIFE_API_KEY 가 등록되어 있어야 합니다.
API_KEY = getattr(settings, 'FINLIFE_API_KEY', 'YOUR_API_KEY')
...
def save_products(request):
    """
    F1303-1: 금감원 API로부터 정기예금 데이터를 가져와 DB에 저장 (중복 방지)
    """
    params = {
        'auth': API_KEY,
    ...
    response = requests.get(f"{BASE_URL}/depositProductsSearch.json", params=params)
    data = response.json()
    ...
    return Response({"message": "예적금 데이터 동기화 완료"}, status=status.HTTP_200_OK)
